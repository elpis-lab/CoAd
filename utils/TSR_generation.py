...
def panda_TSR_parameters(object_details, yaw_buffer, alpha):
    ...
    yaw_tw2_w1 = np.array([alpha*half_side, alpha*half_side, Tw2_w1[2]])*object_dist_check[0:3]
...
def find_yaw_iTSR_set(object_details, problem_details, Tw2_w1):
    ...
    for problem in problem_details:
        ...
        while (yaw_is_covered is False):
            ...
            B12_yaw_intersect = find_TSR_Bounds(Bw, Tw1_0, yaw_1, yaw_2, yaw_buffer, half_side, grasp=problem)
...
def find_iTSR_set(object_details, problem_details, yaw_tw2_w1_dict, yaw_iTSR_set, problem=None, robot_pos=[0.0, 0.0, 0.0]):
    ...
    for grasp_ind, grasp_strategy in enumerate(problem_details):
        ...
        for curr_yaw_interval in tqdm(yaw_iTSR_set[grasp_ind]):
            ...
            while (curr_yaw_is_covered is False):
                ...
                B1_0 = find_TSR_Bounds(Bw, Tw1_0, yaw_1, yaw_2, yaw_buffer, half_side, grasp=grasp_strategy)
                B2_0 = find_TSR_Bounds(Bw, Tw2_0, yaw_1, yaw_2, yaw_buffer, half_side, grasp=grasp_strategy)
                ...
                if (dims[0]==1 and dims[1]==1):
                    ...
                    B3_0 = find_TSR_Bounds(Bw, Tw3_0, yaw_1, yaw_2, yaw_buffer, half_side, grasp=grasp_strategy)
                    B4_0 = find_TSR_Bounds(Bw, Tw4_0, yaw_1, yaw_2, yaw_buffer, half_side, grasp=grasp_strategy)
                ...
                tw1_rvec = np.sqrt((tw1_0[0]-object_position[0])**2 + (tw1_0[1]-object_position[1])**2)

                rmin, rmax = rmin_rmax_from_square_corners(tw1_0, tw2_0, nominal_pose=robot_pos)
