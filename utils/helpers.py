...
def fk_query_env(robot, q, env=0):
    #q = np.asarray(q, dtype=np.float32)
    q = np.array(q, dtype=np.float32, copy=True)
    if q.ndim == 1:
        q = q[None, :]
    
    #print("first genesis fk call")
    w_pos, w_quat = robot.forward_kinematics(q, envs_idx=[env])
    #print("genesis fk call done")
    w_pos_ee  = w_pos[0, 8:]
    w_quat_ee = w_quat[0, 8:]
    return (w_pos_ee, w_quat_ee)


def fk_query(robot, q):

    '''
    returns workspace point from config space point
    tuple: (tensor(finger_1_position, finger_2_position),
            tensor(finger_1_quat, finger_2_quat))
    '''
    #print(q)
    q = np.asarray(q)
    ...
    #print("first genesis fk call")
    w_all_links = robot.forward_kinematics(q)
    #print("genesis fk call done")
    #print(w_all_links[0][8:])
    #print(w_all_links[1][8:])
    #print(w_all_links)
    w_pos = w_all_links[0]
    w_quat = w_all_links[1]
    ...
    w_goal = (w_pos_ee, w_quat_ee)

    return w_goal

def fk_query_batch(robot, q_batch, envs_idx):
    ...
    if envs_idx is not None:
        if len(envs_idx) != B:
            raise ValueError(f"len(envs_idx)={len(envs_idx)} must match batch size B={B}")

        w_pos, w_quat = robot.forward_kinematics(q_batch, envs_idx=envs_idx)
    else:
        w_pos, w_quat = robot.forward_kinematics(q_batch)

    # Your current fk_query does:
    # w_pos_ee = w_pos[0][8:]  -> expects 6 values (2*3)
    # w_quat_ee = w_quat[0][8:] -> expects 8 values (2*4)

    w_pos_ee = w_pos[:, 8:]    # (B, 6)
    w_quat_ee = w_quat[:, 8:]  # (B, 8)

    # reshape into (B, 2, 3) and (B, 2, 4)
    pos_batch = w_pos_ee.reshape(B, 2, 3)
    quat_batch = w_quat_ee.reshape(B, 2, 4)

    return (pos_batch, quat_batch)
