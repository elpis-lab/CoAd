...
class Camera:
    ...
    def setup_socketio_handlers(self):
        @self.sio.on("result")
        def on_pose(data):
            # 1. Get Detections (Dictionary of ID -> 4x4 Matrix)
            raw_detections = data.get("detections", {})

            # Convert lists back to Numpy Arrays
            parsed_detections = {}
            for tag_id, matrix_list in raw_detections.items():
                parsed_detections[int(tag_id)] = np.array(matrix_list)

            self.latest_data = parsed_detections
