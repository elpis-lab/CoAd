...
class PhysicalUR10:
    def __init__(self):
        ...
        self.mode = "push"
        self.robot_home = np.array([1.3, -1.571, 1.2, -1.2, -1.571, -0.271])
    ...
    def get_object_poses(self):
        hand_cam_pose = self.get_object_pose_hand()  # id: 4x4
        ee_transform = self.get_ee_transform()

        obj_poses = {}
        for tag_id, T_cam_obj in hand_cam_pose.items():
            obj_pose = ee_transform @ T_cam_obj
            obj_poses[tag_id] = obj_pose

        return obj_poses

    def two_step_detect(self, object_id: int):
        for i in range(5):
            p = self.get_object_pose_top()[object_id]
            if p is not None:
                break
            time.sleep(0.5)

        self.rtde.move_tool([p[0, 3], p[1, 3] + 0.10, 0.7, 0, np.pi, 0])
        time.sleep(2.0)
        obj_pose = self.get_object_poses()[object_id]
        return obj_pose
