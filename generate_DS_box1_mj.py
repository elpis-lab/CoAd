...
from utils.helpers import save_paths_npy_numeric, to_jsonable
from utils.TSR_generation import panda_TSR_parameters, find_yaw_iTSR_set, find_iTSR_set
#from utils.ik import cover_iTSR, condense_paths3

from concurrent.futures import ProcessPoolExecutor, as_completed
from utils.mj_ik import cover_iTSR, mj_condense, cover_iTSR_test
...
def main(args):
    ...
    yaw_iTSR_set, yaw_to_cover = find_yaw_iTSR_set(object_details, problem_details, Tw2_w1)    
    iTSR_set, iterno = find_iTSR_set(object_details, problem_details, yaw_tw2_w1, yaw_iTSR_set, problem=problem, robot_pos=base_pos)

    if len(iTSR_set)==1:
        print(f"Number of iTSRs: {len(iTSR_set[0])}")
    else:
        for i in range(len(iTSR_set)):
            print(f"Number of {grasping_strategies[i]} iTSRs: {len(iTSR_set[i])}")
    ...
    sv_xml, sv_pos = mujoco_utils.cube_swept_volume_xml(object_size, first_config)
    ...
    if os.path.exists(f"{prefix}/data.npy") and os.path.exists(f"{prefix}/offsets.npy") and os.path.exists(f"{prefix}/keys.npy"):
        if only_condense:
            generate_ds = "n"
        else:
    ...
    if generate_ds.lower()=="y":
        ...
        iTSR_paths, ik_success, plan_success, solve_times, total_plan_times = cover_iTSR_test(model, data, homePos, iTSR_set, object_details, Tew, viewer)
    ...
    if (do_condense):
        ...
        box_geom_list = ['base', 'side_left', 'side_right', 'side_front', 'side_cap', 'side_back']
        obj_geom_list  = ["sv_box1","sv_box2","sv_cyl1","sv_cyl2","sv_cyl3","sv_cyl4"]
        
        obj_geom_list = obj_geom_list + box_geom_list
        root_paths, prefix_map = mj_condense(model, data, prefix, object_details, obj_geom_list, viewer)
