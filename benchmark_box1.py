...
from utils.helpers import save_paths_npy_numeric, to_jsonable, load_store, get_path_by_index

from utils.mj_ik import mj_condense
...
def main(args):
    ...
    panda_dir = "robots/mujoco_menagerie/franka_emika_panda"
    ...
    mujoco_utils.write_relocated_panda(panda_src, panda_dst, base_pos=base_pos, base_quat_wxyz=base_quat)

    # Mujoco initialization
    base_xml, xml_path = mujoco_utils.init_xml(name="box_scene")

    # Prepare box xml
    box_xml = mujoco_utils.yaml_to_xml(yaml_path="configs/scenes/box/scene_box2.yaml")
    ...
    cube_xml = mujoco_utils.cube_to_xml(cube_name, object_position, object_quat, object_size)
    ...
    mujoco_viewer = None
    pathSuffix = MjSuffix(model, data, robot_geoms, obj_geom_list, mujoco_viewer, 200)
    
    planner = omplPlanner(model, data, robot_geoms, log=False)
    ik_solver = mujoco_utils.PandaMinkIK(model, data, robot_geoms, obj_geom_list)
    ...
    while current_run < n_runs:
        ...
        sampled_quat = [math.cos(sampled_yaw/2), 0.0, 0.0, math.sin(sampled_yaw/2)]
        
        if box_xmin <= sampled_pos[0] <= box_xmax and box_ymin <= sampled_pos[1] <= box_ymax:
            in_box = True
        else:
            in_box = False
        ...
        mujoco_utils.move_cube(model, data, cube_name, sampled_pos, sampled_quat)
    
        # Datastructure query
        start_query = time.perf_counter()
        bin_idx = indexer.query_point(sampled_pos[0], sampled_pos[1], sampled_pos[2], sampled_yaw)
        #print(bin_idx)
        if bin_idx is None:
            continue
        #return
        ...
        if len(code_list)>1:
            ...
            suffix = pathSuffix.ik_suffix_single(root_path, prefix_len, bin_goal, ik_solver)
    ...
    print(f"Benchmarking Complete - {n_runs} runs. Skipped {root_samples} root path samples.")
    print(f"RRTConnect success rate: {100.0 * sum(planning_success) / len(planning_success)}")
    ...
    np.save(os.path.join(benchmark_file_path, "ds_times.npy"), np.asarray(ds_times))
    np.save(os.path.join(benchmark_file_path, "ds_success.npy"), np.asarray(ds_success))
    np.save(os.path.join(benchmark_file_path, "ds_suffix_times.npy"), np.asarray(ds_suffix_times))

    np.save(os.path.join(benchmark_file_path, "planning_total_times.npy"), np.asarray(planning_total_times))
    np.save(os.path.join(benchmark_file_path, "planning_solve_times.npy"), np.asarray(planning_solve_times))
    np.save(os.path.join(benchmark_file_path, "planning_success.npy"), np.asarray(planning_success))
