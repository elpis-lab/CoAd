...
def cube_to_xml(
    name, pos, quat, size, fixed=False, rgba=[0.8, 0.8, 0.8, 1]
):  # size = [x, y, z] full lengths, converted to half lengths for xml
...
def cylinder_to_xml(
    name, pos, quat, size, fixed=False, rgba=[0.8, 0.8, 0.8, 1]
):  # size = [r, h], converted to [r, h/2] for xml
    joint_xml = "" if fixed else f'<joint name="{name}_free" type="free"/>'
...
PANDA_JOINTS_9 = [f"joint{i}" for i in range(1, 8)] + [
    "finger_joint1",
    "finger_joint2",
]
...
def mj_ik_multilink_until_tol(
    ...
    configuration = Configuration(model, data.qpos[:])
    ...
    for link_target in link_targets:
        ...
        if target_quat is not None:
            ...
            target = SE3.from_translation(target_pos) @ SE3.from_rotation(
                rotation
            )
            ori_cost = 1.0
        else:
        ...
        task = FrameTask(
            ...
            gain=(
                gain if single else None
            ),  # mink ignores None; or just always pass gain
        )
    ...
    for it in range(max_iters):
        ...
        for link_target in link_targets:
            ...
            pos_err_i, ang_err_i, _, _ = ee_pose_error(
                model, data, body_name, target_pos, target_quat
            )
...
def mj_ik_multilink(
    ...
    hand = [
        "panda_hand_col",
        "panda_leftfinger_colmesh",
        "panda_rightfinger_colmesh",
    ]
    pads = [
        "panda_leftpad_1",
        "panda_leftpad_2",
...
class PandaMinkIK:
    def __init__(
        ...
        self.finger_freezing_task = DofFreezingTask(
            model=model, dof_indices=[7, 8]
        )
    ...
    def solve(self, link_targets, seed, ignore_col=False, single=False):
        ...
        for lt in link_targets:
            ...
            target = SE3.from_translation(lt["pos"]) @ SE3.from_rotation(
                rotation
            )
        ...
        limits = (
            [self.config_limit]
            if ignore_col
            else [self.config_limit, self.collision_limit]
        )
...
class MujocoViewer:
    ...
    def play_qpos_traj(
        self, qpos_traj, dt=None, wait_for_enter=True, loop=False
    ):
