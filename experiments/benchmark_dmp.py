...
def evaluate_graph(
    ...
    for adaptation in adaptations:
        ...
        lib_sizes[adaptation] = len(root_data)
        ...
        if adaptation == "dmp":
            ...
            raise ValueError(f"Only 'dmp' is supported in this benchmark script, got: {adaptation}")

        adapters.append(adapter)

    solved_keys = solved_task_paths_keys

    print(f"\n=== Evaluating DMP over {num_samples} samples ===")
    num_tested = 0
    ...
    while num_tested < num_samples:
        ...
        recovered_key = indexers[0].query_point(sample)
        ...
        if recovered_key_adapt is None:
            ...
            adaptation_lengths[adaptation].append(
                traj_len(adapted_path) if curr_success else np.nan
            )
    ...
    lengths = np.asarray(adaptation_lengths[adaptation], dtype=float)

    times_succ = times[succ]

    mean_time_ms = np.nanmean(times_succ) * 1000 if np.any(succ) else np.nan
    std_time_ms = np.nanstd(times_succ, ddof=1) * 1000 if np.sum(succ) > 1 else 0.0
    mean_length = np.nanmean(lengths)
    std_length = np.nanstd(lengths, ddof=1) if np.sum(np.isfinite(lengths)) > 1 else 0.0
    success_rate = np.mean(succ) * 100
    ...
    results_path = f"data/dmp_results_{args.robot}_{args.env}.npz"
    results = {
        "adaptations": {
            "success": {"dmp": np.asarray(adaptation_success["dmp"], dtype=bool)},
            "times": {"dmp": np.asarray(adaptation_times["dmp"], dtype=float)},
            "lengths": {"dmp": np.asarray(adaptation_lengths["dmp"], dtype=float)},
        }
    }
...
def main(args):
    ...
    adaptations_found = ["dmp"]

    if args.env == "real" and args.robot == "ur10":
        suffix = f"{args.ik}_{args.planner}_dmp_100"
    else:
        suffix = f"{args.ik}_{args.planner}_dmp_{args.n_neighbors}"

    root_path = f"{folder}/root_paths_{suffix}.pkl"
    map_path = f"{folder}/key_to_root_{suffix}.pkl"

    if not (os.path.exists(root_path) and os.path.exists(map_path)):
        raise FileNotFoundError(f"DMP data not found for problem: {args.robot} in {args.env}")
