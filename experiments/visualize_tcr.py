...
from experiments.visualize_goals import (
    move_object, positive_seconds, sample_from_tcr,
    set_camera, split_tcr,
)
...
def sample_delta(bounds, planar=False, corner=None):
    ...
    half = [bounds.x - bounds.position_slack,
    ...
    if corner is not None:
        ...
        for bit, axis in enumerate(axes):
            displacement[axis] = half[axis] * (1 if corner & (1 << bit) else -1)
...
def fits_tsr(ee, pose, offset, bounds, details=None):
    delta = np.linalg.solve(reference_pose(pose, details), ee @ np.linalg.inv(offset))
    tilt = np.arccos(np.clip(delta[2, 2], -1, 1))
    return (np.all(np.abs(delta[:3, 3]) <= np.array([bounds.x, bounds.y, bounds.z]) + 1e-12)
...
def collision_geoms(env, robot, face):
    ...
    if face:
        ...
        name = "microwave_object" if env.environment_name == "microwave" else "cube_object"
    bodies = {env.model.body(name).id}
    for body in range(env.model.nbody):
        if env.model.body_parentid[body] in bodies:
            bodies.add(body)
    return set(robot.robot_geoms) | set(np.flatnonzero(np.isin(env.model.geom_bodyid, list(bodies))))


def has_penetration(data, geoms, tolerance=1e-6):
    """Allow touching a support/grasp surface, but reject penetration > 1 micron."""
    return any(contact.dist < -tolerance and
               (contact.geom1 in geoms or contact.geom2 in geoms)
               for contact in data.contact[:data.ncon])


def accept_sample(env, robot, face, pose, q, offset, bounds, geoms, details=None):
...
def reference_tasks(env):
    ...
    clipped = []
    for region, x, y in itertools.product(regions, variation["x"], variation["y"]):
    ...
    for face, xy, z, dz, yaw, door in itertools.product(
        faces, regions, variation["z"], env.env_details.get("z_correction", [0]),
        variation["yaw"], variation.get("door", [None]),
    ):
        if face:
            height = env.object_details["size"][("yz", "zx", "xy").index(face)] / 2
            z = [height, height]
...
def find_anchor(env, robot, solver, tasks, cells, attempts=200):
    if not tasks:
        raise ValueError("No object placement ranges intersect the task domain")
    for attempt in range(attempts):
        for _ in range(1000):
            face, key = split_tcr(tasks[np.random.randint(len(tasks))])
            pose = sample_from_tcr(key)
            distance = np.linalg.norm(np.asarray(pose[:2]) - env.env_details["robot_pos"][:2])
            if env.env_details["inner_rad"] <= distance <= env.env_details["outer_rad"]:
                break
        else:
            raise RuntimeError("Could not sample a pose in the robot's task annulus")
        cell = cells[face or "default"]
        offsets = [np.asarray(T) for T in cell["ee_offsets"]]
        offset = offsets[np.random.randint(len(offsets))]
        details = env.object_details if env.environment_name == "microwave" else None
        target = reference_pose(pose, details) @ offset
        ok, q = solver.solve(matrix_to_flat(target), current=env.data.qpos.copy(),
                             random_current=attempt % 3 == 2, max_iters=200,
                             pos_tol=1e-6, rot_tol=1e-5)
        if ok:
            reason = accept_sample(env, robot, face, pose, q, offset,
                                   TSRBounds(**cell["tsr_bounds"]),
                                   collision_geoms(env, robot, face), details)
            if reason is None:
                return face, pose, cell, offset
    raise RuntimeError(f"No collision-free TSR anchor found in {attempts} IK attempts")
...
def precompute_samples(env, robot, solver, anchor, modes, samples):
    ...
    try:
        for mode in modes:
            ...
            print(f"Preparing {mode.upper()}: {samples} candidates...", flush=True)
            for count in range(samples):
                delta = sample_delta(bounds, planar=mode == "tcr",
                ...
                if mode == "tsr":
                    ...
                    ok, q = solver.solve(matrix_to_flat(target),
                                         current=env.data.qpos.copy(), max_iters=60,
                                         pos_tol=1e-6, rot_tol=1e-5)
                    reason = (accept_sample(env, robot, face, pose, q, grasp,
                                            bounds, geoms, details) if ok else "IK")
                else:
                    candidate = inverse_object_pose(fixed_ee, offset, delta, pose, details)
                    reason = accept_sample(env, robot, face, candidate, anchor_q,
                                           offset, bounds, geoms, details)
                if reason:
                    rejected[reason] += 1
                else:
                ...
                if (count + 1) % 25 == 0:
                    print(f"  {mode.upper()}: {count + 1}/{samples} checked; "
                          f"{len(frames)} accepted", flush=True)
            print(f"{mode.upper()} ready: {len(frames)} accepted, rejected={rejected}")
            if not frames:
                raise RuntimeError(f"No valid {mode.upper()} samples in {samples} attempts; "
        ...
        with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
            ...
            while viewer.is_running():
                ...
                if now >= next_sample:
                    ...
                    if frame_index == 0:
                        print(f"Playing {mode.upper()}: {len(buffers[mode])} validated samples")
                    with viewer.lock():
                ...
                viewer.sync()
                time.sleep(min(1 / 120, max(0, next_sample - time.monotonic())))
    finally:
...
def main(args):
    ...
    try:
        ...
        metadata_path = Path(get_data_folder(args.env, args.robot)) / "task_set.tcr.json"
        if metadata_path.exists():
            with metadata_path.open() as stream:
                cells = json.load(stream)["cells"]
        else:
            print("No saved TSR metadata; constructing the environment's TSR bounds.")
            env.construct_tcr()
        ...
        print("Preparing TSR interiors and corners; rejecting penetrations over 1 micron.")
        print("Sample checks do not certify the entire continuous TSR boundary.")
        print("Finding a collision-free anchor before opening the viewer...", flush=True)
        anchor = find_anchor(env, robot, solver, tasks, cells)
        modes = ("tsr", "tcr") if args.mode == "both" else (args.mode,)
        buffers = precompute_samples(env, robot, solver, anchor, modes, args.samples)
        print("Preparation complete. Opening viewer; close it to stop playback.")
...
def parse_arguments():
    ...
    parser.add_argument("--mode", choices=("tsr", "tcr", "both"), default="both")
    parser.add_argument("--interval", type=positive_seconds, default=0.05,
