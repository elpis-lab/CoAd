...
from src.condense_paths import slerp
from utils.mujoco_utils import move_swept_volume, set_panda_qpos, robot_in_contact
from utils.mujoco_utils import MujocoViewer, mj_fk, mj_ik_multilink

class MjPrefix:
    def __init__(self, model, data, robot_geoms, mujoco_viewer, path_length, min_suffix_factor=0.1):
...
class MjSuffix:
    def __init__(self, model, data, robot_geoms, obj_geom_list, mujoco_viewer, path_length):
        self.path_length = path_length
        self.model = model
        self.data = data
        self.robot_geoms = robot_geoms
    ...
    def ik_suffix(self, root_path, prefix_length, q_goal):
        ...
        #for i in range(1):
            ...
            q_ik = mj_ik_multilink(self.model, self.data, link_targets, prefix[-1], self.robot_geoms, self.obj_geom_list)
    ...
    def ik_suffix_single(self, root_path, prefix_length, q_goal, ik_solver):
        ...
        seed = root_path[prefix_length-1] if prefix_length > 0 else root_path[0]
        goal_w = mj_fk(self.model, self.data, q_goal)
