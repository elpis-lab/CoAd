...
from helpers.helpers import rpy_to_R, load_store, get_path_by_index

# from src.condense_paths import PrefixBuilder, SuffixBuilder, SparseBoxGrid4D
from src.visualization import Plotter

from helpers.mujoco_utils import get_robot_collision_geom_ids, MujocoViewer
...
from helpers.mujoco_utils import (
    ...
    PandaMinkIK,
    get_panda_qpos_idxs,
    mj_ik_multilink_until_tol,
)

from helpers.mujoco_utils import (
    qpos_within_limits,
    randomize_seed_within_limits,
)
...
def mj_condense(
    mj_model, mj_data, filename_prefix, object_details, obj_geom_list, viewer
):
    """
    ...
    mujocoViewer = (
        MujocoViewer(mj_model, mj_data, robot_geoms) if viewer else None
    )
    ...
    while uncovered_key_indices:
        ...
        pathPrefix = MjPrefix(
            mj_model, mj_data, robot_geoms, mujocoViewer, len(path)
        )
        pathSuffix = MjSuffix(
            mj_model,
            mj_data,
