...
def find_B0(Bw, Tew, Tw0):
    ...
    c, s = np.cos(yaw_ang), np.sin(yaw_ang)
    R = np.array([
        [c, -s],
        [s,  c]
    ])
...
def main():
    ...
    '''
    debug_lbounds = [
        [object_lower[0]-object_size[0]/2, object_lower[1]-object_size[1]/2 + debug_offset, object_lower[2]-object_size[2]/2],
        [object_lower[0]+object_size[0]/2, object_lower[1]+object_size[1]/2 + debug_offset, object_lower[2]+object_size[2]/2]
    ]
    debug_ubounds = [
        [object_upper[0]-object_size[0]/2, object_upper[1]-object_size[1]/2 + debug_offset, object_upper[2]-object_size[2]/2],
        [object_upper[0]+object_size[0]/2, object_upper[1]+object_size[1]/2 + debug_offset, object_upper[2]+object_size[2]/2]
    ]
    '''

    debug_lbounds = [
        [object_lower[0]-object_radius, object_lower[1]-object_radius, object_lower[2]-object_height/2],
        [object_lower[0]+object_radius, object_lower[1]+object_radius, object_lower[2]+object_height/2]
    ]
    debug_ubounds = [
        [object_upper[0]-object_radius, object_upper[1]-object_radius, object_upper[2]-object_height/2],
        [object_upper[0]+object_radius, object_upper[1]+object_radius, object_upper[2]+object_height/2]
    ]

    debug_lubounds = [
        [object_lower[0]-object_radius, object_upper[1]-object_radius, object_lower[2]-object_height/2],
        [object_lower[0]+object_radius, object_upper[1]+object_radius, object_lower[2]+object_height/2]
    ]
    debug_ulbounds = [
        [object_upper[0]-object_radius, object_lower[1]-object_radius, object_upper[2]-object_height/2],
        [object_upper[0]+object_radius, object_lower[1]+object_radius, object_upper[2]+object_height/2]
    ]
    ...
    while (yaw_is_covered is False):
        first_pos = [object_position[0]-object_dist[0],
        ...
        for i, key in enumerate(iTSR_set):
            ...
            while alive.any():
                ...
                if new_idx.size:
                    ...
                    #sample_pos[idx, :] = Ttargets[:, :3, 3]
                    #sample_quat[idx, :] = np.asarray(
                    #    [to_genesis_quat(q) for q in q_xyzw],
                    #    dtype=np.float32
                    #)
                ...
                #if(ik_attempt == 0):
                #    print("First IK Attempt:")
                #    print(q_batch)
                #elif(ik_attempt == 1):
                #    print("Second IK Attempt:")
                #    print(q_batch)
                ...
                #print(f"q_delta means: {q_delta}")
                #print(q_delta_means[idx])
                ...
                #print(np.nonzero(alive)[0])
                #input("Proceed?")
                ik_attempt += 1

            print(f"IK complete: {i}-{i+n_envs}")
            homePosStack = np.tile(homePos, (25, 1))
            #path = robot.plan_path(
            #        qpos_goal = q_batch,
            #        qpos_start = homePosStack,
            #        num_waypoints = 200,
            #        planner = 'RRTConnect',
            #        envs_idx = np.array(list(range(n_envs)))
            #    )
            ...
            for b, k in enumerate(current_keys):
                ...
                q_goal  = np.asarray(q_batch[b], dtype=np.float32)
    ...
    for i, key in enumerate(iTSR_set):
        ...
        if(i==0):
            print(key)
            volumes = find_swept_volume(object_size, key, volumes=None)
            plane = scene.add_entity(gs.morphs.Plane())
            scene.build()
        else:
            volumes = find_swept_volume(object_size, key, volumes=volumes)  
        ...
        while(inCollision):
            ...
            lo = np.minimum(iTSR[:, 0], iTSR[:, 1])  
            hi = np.maximum(iTSR[:, 0], iTSR[:, 1])
            intersect_sample = rng.uniform(lo, hi)
            intersect_sample[lo == hi] = lo[lo == hi]
            #print(intersect_sample)

            intersect_sample_R = rpy_to_R(intersect_sample[3], intersect_sample[4], intersect_sample[5])
            Tintersect_0 = np.eye(4)
            Tintersect_0[:3, :3] = intersect_sample_R
            Tintersect_0[:3, 3] = [intersect_sample[0], intersect_sample[1], intersect_sample[2]]

            Tint_sample = Tintersect_0 @ Tew

            num_links = robot.n_links
            ee_link1_idx = num_links-1-1
            ee_link2_idx = num_links-1

            #sample_quat = to_genesis_quat(R.from_matrix(Tsample0[:3, :3]).as_quat())
            #sample_pos = [Tsample0[0, 3], Tsample0[1, 3], Tsample0[2, 3]]

            sample_quat = to_genesis_quat(R.from_matrix(Tint_sample[:3, :3]).as_quat())
            sample_pos = [Tint_sample[0, 3], Tint_sample[1, 3], Tint_sample[2, 3]]


            q, ik_error = robot.inverse_kinematics_multilink(
                links = [robot.links[ee_link1_idx], robot.links[ee_link2_idx]],
                poss = [sample_pos, sample_pos],
                quats = [sample_quat, sample_quat],
                return_error = True,
                pos_tol = 1e-10
            )
            if q is not None:
            ...
            link1_error = ik_error.tolist()[0]
            link2_error = ik_error.tolist()[1]
            mean_error = [(x + y)/2 for x, y in zip(link1_error, link2_error)]

            #print("IK Error: ", mean_error)
            ...
            if collisions.size > 0:
                ...
                inCollision = False
                print(f"TSR Number: {i+1}/{len(iTSR_set)} - IK Attempts: {ik_attempt}")
            #input("Proceed to next sample?")
