...
def find_swept_volume(scene, object_dims, object_configs, volumes=None, env_idx=None):
    ...
    if (volumes is None):
        ...
        if env_idx is not None:
            ...
            sv_c1.set_pos(pos=[[x_lower, y_lower, z]], envs_idx=env_idx)
            sv_c2.set_pos(pos=[[x_lower, y_upper, z]], envs_idx=env_idx)
            sv_c3.set_pos(pos=[[x_upper, y_lower, z]], envs_idx=env_idx)
            sv_c4.set_pos(pos=[[x_upper, y_upper, z]], envs_idx=env_idx)
        else:
            sv_b1.set_pos(pos=[(x_upper+x_lower)/2, (y_upper+y_lower)/2, z], envs_idx=env_idx)
            sv_b2.set_pos(pos=[(x_upper+x_lower)/2, (y_upper+y_lower)/2, z], envs_idx=env_idx)
            sv_c1.set_pos(pos=[x_lower, y_lower, z], envs_idx=env_idx)
...
def to_numpy_array(x, dtype=None):
    ...
    # Case 2: list/tuple of tensors (this is your `path` from omplPlan)
    if isinstance(x, (list, tuple)) and len(x) > 0 and isinstance(x[0], torch.Tensor):
        arr = torch.stack([t.detach().cpu() for t in x]).numpy()
        if dtype is not None:
            return arr.astype(dtype, copy=False)
        return arr

    # Case 3: everything else (lists, empty lists, numpy arrays, etc.)
...
def pack_paths_from_dict(path_dict, *, path_dtype=np.float32, key_dtype=None):
    ...
    raw_paths = [
        to_numpy_array(path_dict[k], dtype=path_dtype)
        for k in keys_list
    ]
...
def rmin_rmax_from_square_corners(tw1, tw2, nominal_pose=[0, 0, 0]):
    ...
    # Ensure proper min/max ordering in translated frame
    x_min, x_max = min(x1, x2), max(x1, x2)
    y_min, y_max = min(y1, y2), max(y1, y2)

    # ---- r_min: closest distance from (0,0) to the translated square ----
    if (x_min <= 0.0 <= x_max) and (y_min <= 0.0 <= y_max):
        r_min = 0.0
    else:
        nearest_x = np.clip(0.0, x_min, x_max)
        nearest_y = np.clip(0.0, y_min, y_max)
        r_min = np.sqrt(nearest_x**2 + nearest_y**2)

    # ---- r_max: farthest distance from (0,0) to the translated square ----
    corners = np.array([
        [x_min, y_min],
        [x_min, y_max],
        [x_max, y_min],
...
def load_static_obstacles(query_data, scene_data, variation_data, scene, world_var=False):
    ...
    goal_queries = query_data.get('goal_queries')
    query_num = len(goal_queries)
    selected_query_index = random.randint(0, query_num-1)
    selected_query = goal_queries[selected_query_index]

    objects = selected_query['objects']
    objects_len = len(objects)
    
    selected_object_index = random.randint(0, objects_len-1)
    selected_object = objects[selected_object_index] #Box1
    selected_tag = selected_query['tag'] #Top

    offset_pos = np.array(selected_query['offset']['position'])
    offset_ori = np.array(selected_query['offset']['orientation'])

    #print(selected_object)
    #print(selected_tag)

    for i in range(len(variation_data)):
        curr_names = variation_data[i]['names']
        
        for j in range(len(curr_names)):
            if(curr_names[j]==selected_object):
                pos_var = variation_data[i]['position']
                ori_var = variation_data[i]['orientation']
                break
            else:
                pos_var = None
                ori_var = None
    
    if pos_var is not None and ori_var is not None:
        object_dist = [pos_var[0], pos_var[1], pos_var[2], ori_var[2]]
        #print(f"object dist: {object_dist}")
        object_dist_check = np.sign(np.array(object_dist))
        #print(f"object dist check: {object_dist_check}")
    else:
        object_dist = None
        object_dist_check = None
    
    print(f"Object dist: {object_dist}")


    collision_objects = scene_data["world"]["collision_objects"]
    zmin = 0

    for i in range(len(collision_objects)):
        curr_ID = collision_objects[i]['id']
        #if (yaml_data.isYAML):
        curr_pose = collision_objects[i]['primitive_poses']
        curr_dim = collision_objects[i]['primitives'][0]['dimensions']
        curr_type = collision_objects[i]['primitives'][0]['type']
        #else:
        #    curr_pose = collision_objects[i]['mesh_poses']
        if curr_ID == selected_object:
            selected_pose = curr_pose
            selected_dim = curr_dim
            selected_type = curr_type
        else:
            if curr_type == "box":
                #print(to_genesis_quat(curr_pose[0]['orientation']))
                #print()
                scene.add_entity(gs.morphs.Box(pos=curr_pose[0]['position'], size=curr_dim, quat=to_genesis_quat(curr_pose[0]['orientation']), fixed=True))
                currMin = curr_pose[0]['position'][2]-curr_dim[2]/2
            else: #cylinder
                scene.add_entity(gs.morphs.Cylinder(pos=curr_pose[0]['position'], quat=to_genesis_quat(curr_pose[0]['orientation']), fixed=True, radius=curr_dim[1], height=curr_dim[0]))
                currMin = curr_pose[0]['position'][2]-curr_dim[0]/2
            
            if currMin<zmin:
                zmin = currMin
    
    scene.add_entity(gs.morphs.Plane(pos=(0.0, 0.0, zmin)))
...
def main():
    ...
    robotURDF = "robowflex_resources/panda/urdf/panda.urdf"
    scenepath = "configs/scenes/box/"

    scenefile = scenepath + "scene_box.yaml"
    variationfile = scenepath + "variation_box.yaml"
    queryfile = scenepath + "easy_queries_box.yaml"

    with open(scenefile, 'r') as f:
        scene_data = yaml.safe_load(f)
    
    with open(variationfile, 'r') as f:
        variation_data = yaml.safe_load(f)
    
    with open(queryfile, 'r') as f:
        queries_data = yaml.safe_load(f)

    load_static_obstacles(queries_data, scene_data, variation_data, scene, world_var=False)

    robot_base_position = (0, 0, 0)
    #robot_base_position = [1.05, 0, 0.7]
    robot_base_position = [0.15, 0.0, 1.02]
    robot_base_position = [0.15, 0.0, 0.75]
    ...
    nominal_object_pose = [
        ...
        box_level+object_size[2]/2
    ]

    object_position = nominal_object_pose

    #robot_clearance = 0.55
    #reachable_ws = 0.551
    robot_clearance = 0.3
    reachable_ws = 0.8

    object_dist = [0.1, 0.1, 0, np.pi]
    object_dist = [reachable_ws, reachable_ws, 0, 0.0*np.pi]
    ...
    if object_dist[3]!=0:
        ...
        print("Cylindrical object. TSR yaw = -pi to +pi")
        Bw = np.array([
            [-del_geom_x, del_geom_x],
            [-del_geom_y, del_geom_y],
            [0, 0],
    ...
    for curr_yaw_interval in yaw_iTSR_set:
        ...
        while (curr_yaw_is_covered is False):
            ...
            rmin, rmax = rmin_rmax_from_square_corners(tw1_0, tw2_0, nominal_pose=nominal_object_pose)
            in_sample_space = (rmin <= reachable_ws) and (rmax >= robot_clearance)

            if (box_window[0][0] < tw1_0[0]-can_radius) and (tw1_0[0]+can_radius < box_window[0][1]) and (box_window[1][0] < tw1_0[1]-can_radius) and (tw1_0[1]+can_radius < box_window[1][1]):
                #in_box = True
                if (box_window[0][0] < tw2_0[0]-can_radius) and (tw2_0[0]+can_radius < box_window[0][1]) and (box_window[1][0] < tw2_0[1]-can_radius) and (tw2_0[1]+can_radius < box_window[1][1]):
                    in_box = True
                else:
                    in_box = False
            else:
        ...
        try:
            for i, key in enumerate(iTSR_set):
                ...
                if(i==0):
                    ...
                    scene.build(n_envs=n_envs, env_spacing=(5.0, 5.0))
    ...
    prefix = f"TSRs/box_limit2/{robot_clearance}_{reachable_ws}_{round(object_dist[3], 2)}"
